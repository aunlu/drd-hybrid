...
import com.devexperts.drd.agent.Comparison;
...
public class ConcurrentWeakHashMap<K, V> extends AbstractMap<K, V>
    ...
    final Comparison keyComparison;
    ...
    static final class Segment<K, V> extends ReentrantLock implements Serializable {
        ...
        transient final AtomicInteger weakClearedCount = new AtomicInteger(0);

        final Comparison keyComparison;

        Segment(int initialCapacity, float lf, Comparison keyComparison) {
            loadFactor = lf;
        ...
        V get(Object key, int hash) {
            if(count != 0) { // read-volatile
                ...
                while(e != null) {
                    if(e.hash == hash && keyComparison.equals(key, e.keyRef.get())) {
                        V v = e.value;
                        if(v != null)
                            return v;
        ...
        boolean replace(K key, int hash, V oldValue, V newValue) {
            ...
            try {
                removeStale();
                HashEntry<K, V> e = getFirst(hash);
                while(e != null && (e.hash != hash || !keyComparison.equals(key, e.keyRef.get())))
                    e = e.next;
        ...
        V remove(Object key, int hash, Object value, boolean weakRemove) {
            ...
            try {
                if (weakRemove) {
                    weakClearedCount.incrementAndGet();
                }
                if(!weakRemove)
                    removeStale();
                int c = count - 1;
                ...
                while(e != null && (!weakRemove || key != e.keyRef)
                        && (e.hash != hash || !keyComparison.equals(key, e.keyRef.get())))
                    e = e.next;
     ...
     * nonpositive.
     */
    public ConcurrentWeakHashMap(int initialCapacity, float loadFactor, int concurrencyLevel, Comparison keyComparison) {
        ...
        for(int i = 0; i < this.segments.length; ++i)
            this.segments[i] = new Segment<K, V>(cap, loadFactor, keyComparison);
    }
     ...
     * @since 1.6
     */
    public ConcurrentWeakHashMap(int initialCapacity, float loadFactor, Comparison keyComparison) {
        this(initialCapacity, loadFactor, DEFAULT_CONCURRENCY_LEVEL, keyComparison);
    }
     ...
     */
    public ConcurrentWeakHashMap(int initialCapacity, Comparison keyComparison) {
        this(initialCapacity, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, keyComparison);
    }
     ...
     */
    public ConcurrentWeakHashMap(Comparison keyComparison) {
        this(DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, keyComparison);
    }
     ...
     * @param m the map
     */
    public ConcurrentWeakHashMap(Map<? extends K, ? extends V> m, Comparison keyComparison) {
        this(Math.max((int) (m.size() / DEFAULT_LOAD_FACTOR) + 1, DEFAULT_INITIAL_CAPACITY), DEFAULT_LOAD_FACTOR, DEFAULT_CONCURRENCY_LEVEL, keyComparison);
        putAll(m);
    }
    ...
    public int getWeakClearedCount() {
        int count = 0;
        for (Segment s : segments) {
            count += s.weakClearedCount.getAndSet(0);
        }
        return count;
    }
    ...
    public V get(Object key) {
        int hash = hash(keyComparison.hashCode(key));
    ...
    static class SimpleEntry<K, V> implements Entry<K, V>, java.io.Serializable {
        ...
        private final Comparison keyComparison;
        private final K key;
        private V value;

        public SimpleEntry(K key, V value, Comparison keyComparison) {
            this.key = key;
            this.value = value;
            this.keyComparison = keyComparison;
        }

        public SimpleEntry(Entry<? extends K, ? extends V> entry, Comparison keyComparison) {
        ...
        public boolean equals(Object o) {
            ...
            return keyComparison.equals(key, e.getKey()) && eq(value, e.getValue());
        }

        public int hashCode() {
            return (key == null ? 0 : keyComparison.hashCode(key)) ^ (value == null ? 0 : value.hashCode());
        }
